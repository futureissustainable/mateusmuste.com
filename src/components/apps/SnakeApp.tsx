...
export function SnakeApp({ onAchievement }: SnakeAppProps) {
  ...
  const snake = useRef<Position[]>([{ x: 15, y: 10 }]);
  const food = useRef<Position>({ x: 20, y: 10 });
  const dir = useRef<Position>({ x: 1, y: 0 });
  const nextDir = useRef<Position>({ x: 1, y: 0 });
  ...
  const update = useCallback(() => {
    ...
    // Snake - black with SUDOUNLOCK letters
  ...
  // Touch controls
  const handleDirection = useCallback((newDir: string) => {
    if (!runningRef.current) return;
    ...
    >
      ...
      <div className="flex-grow flex items-center justify-center p-2 md:p-4 bg-gray-50 overflow-hidden">
        <div
          ...
          style={{ width: 'min(100%, 600px)', aspectRatio: '3/2' }}
        >
          <canvas
            ref={canvasRef}
            width={600}
      ...
      <div className="app-footer">
        <span className="text-gray-500 text-xs">EAT FOOD - GROW LONGER - DON&apos;T DIE</span>
      </div>
    </div>
  );
}
