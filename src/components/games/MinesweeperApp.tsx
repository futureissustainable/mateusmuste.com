...
export const MinesweeperApp = memo(({ onAchievement }: MinesweeperAppProps) => {
  ...
  const getBackgroundLetter = (r: number, c: number) => {
    const idx = r * COLS + c;
    return HIDDEN_MESSAGE[idx] || '';
  };

  const initGame = () => {
    const newGrid: number[][] = Array(ROWS).fill(null).map(() => Array(COLS).fill(0));
    const newRevealed: boolean[][] = Array(ROWS).fill(null).map(() => Array(COLS).fill(false));
    const newFlagged: boolean[][] = Array(ROWS).fill(null).map(() => Array(COLS).fill(false));
...
MinesweeperApp.displayName = 'MinesweeperApp';
